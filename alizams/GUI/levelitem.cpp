...
LevelItem::LevelItem(QGraphicsRectItem * item, int role, HistogramView * v) : QGraphicsItem(NULL)
{
...
QRectF LevelItem::boundingRect() const
{
	...
	case LeftHandle:
		{
			QPointF point(m_item->boundingRect().left()-pos().x(), m_item->boundingRect().top() + m_item->boundingRect().height()/2.0);
			r = QRectF(point-QPointF(12.0, m_item->boundingRect().height()/2.0), QSize(24, m_item->boundingRect().height()));
		}
		break;
	case RightHandle:
		{
			QPointF point(m_item->boundingRect().right()-pos().x(), m_item->boundingRect().top() + m_item->boundingRect().height()/2.0);
			r= QRectF(point-QPointF(12.0, m_item->boundingRect().height()/2.0), QSize(24, m_item->boundingRect().height()));
		}
		break;
	}
		...
		case LeftHandle:
			{
				...
				{
					newRect.setLeft(m_item->rect().left()+dx<m_item->rect().right() ? m_item->rect().left()+dx : m_item->rect().right());
		...
		case RightHandle:
			{
				if ((m_item->rect().width()+dx) <= view->scene()->sceneRect().width())
				{
					newRect.setRight(m_item->rect().right()+dx>m_item->rect().left() ? m_item->rect().right()+dx : m_item->rect().left());
					m_item->setRect(newRect);
