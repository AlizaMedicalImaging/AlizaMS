...
CAPICryptographicMessageSyntax::~CAPICryptographicMessageSyntax()
{
  for (std::vector<PCCERT_CONTEXT>::iterator it = certifList.begin(); it != certifList.end(); ++it)
  {
    ...
    {
      mdcmWarningMacro( "Error at releasing certificate context: " << std::hex << GetLastError());
    }
    */
  }
  if (hRsaPrivK)
    CryptDestroyKey(hRsaPrivK);
  if (!CryptReleaseContext(hProv, 0))
  {
    mdcmWarningMacro("Error when releasing context: 0x" << std::hex << GetLastError());
  }
}

// http://stackoverflow.com/questions/11709500/capi-does-not-support-password-based-encryption-pbe-encryption
bool
CAPICryptographicMessageSyntax::SetPassword(const char *, size_t)
{
  mdcmWarningMacro("CAPI does not support Password Based Encryption.");
  return false;
}
...
CAPICryptographicMessageSyntax::ParseKeyFile(const char * filename)
{
  ...
  {
    mdcmErrorMacro("CryptImportKey failed with error 0x" << std::hex << GetLastError());
    goto err;
  }
  if (hRsaPrivK)
    CryptDestroyKey(hRsaPrivK);
...
void
CAPICryptographicMessageSyntax::SetCipherType(CryptographicMessageSyntax::CipherTypes type)
{
  cipherType = type;
}

CryptographicMessageSyntax::CipherTypes
CAPICryptographicMessageSyntax::GetCipherType() const
{
  return cipherType;
}

bool
CAPICryptographicMessageSyntax::Encrypt(char * output, size_t & outlen, const char * array, size_t len) const
{
  CRYPT_ALGORITHM_IDENTIFIER EncryptAlgorithm = { 0 };
  const char *               objid = GetCipherObjId();
  if (!objid)
  {
    mdcmErrorMacro("Could not GetCipherObjId");
    return false;
  }
  ...
  {
    mdcmErrorMacro("No recipients certificates loaded.");
    return false;
  }
                           ...
                           (DWORD *)&outlen))
  {
    ...
    mdcmErrorMacro("Couldn't encrypt message. CryptEncryptMessage failed with error 0x" << std::hex << dwResult);
    if (dwResult == CRYPT_E_UNKNOWN_ALGO)
    {
      mdcmErrorMacro("Unknown encryption algorithm. If on Windows XP please use only 3DES.");
    }
    return false;
  }
  return true;
...
CAPICryptographicMessageSyntax::Decrypt(char * output, size_t & outlen, const char * array, size_t len) const
{
  ...
  if (hRsaPrivK == 0)
  {
    mdcmErrorMacro("No private key loaded loaded.");
    return false;
  }
  if (!(hMsg = CryptMsgOpenToDecode(CRYPT_ASN_ENCODING | X509_ASN_ENCODING | PKCS_7_ASN_ENCODING,
                                    0,
  ...
  {
    mdcmErrorMacro("MsgUpdate failed with error 0x" << std::hex << GetLastError());
    goto err;
  }
  DWORD dwMessageType, cbMessageTypeLen;
  if (!CryptMsgGetParam(hMsg, CMSG_TYPE_PARAM, 0, &dwMessageType, &cbMessageTypeLen))
  {
    mdcmErrorMacro("CryptMsgGetParam CMSG_TYPE_PARAM failed with error 0x" << std::hex << GetLastError());
    goto err;
  }
  if (dwMessageType != CMSG_ENVELOPED)
  {
    mdcmErrorMacro("Wrong message type ( != CMSG_ENVELOPED )");
    goto err;
  }
  ALG_ID kekAlg;
  DWORD  kekAlgLen;
  ...
  {
    mdcmErrorMacro("MsgGetParam KP_ALGID failed with error 0x" << std::hex << GetLastError());
    goto err;
  }
  if (kekAlg != CALG_RSA_KEYX)
  {
    mdcmErrorMacro("Key encryption algorithm is not RSA.");
    goto err;
  }
  ...
  {
    mdcmErrorMacro("Decode CMSG_RECIPIENT_COUNT_PARAM failed with error 0x" << std::hex << GetLastError());
    goto err;
  }
  DWORD cekLen;
  {
    ...
    for (DWORD i = 0; i < nrOfRecipeints; ++i)
    {
      ...
      {
        mdcmErrorMacro("MsgGetParam CMSG_CMS_RECIPIENT_INFO_PARAM failed with error 0x" << std::hex << GetLastError());
        goto err;
      }
      DWORD rsaPadding = 0;
      ...
      if ((foundRecipient = CryptDecrypt(hRsaPrivK, 0, TRUE, rsaPadding, cek, &cekLen)))
        break;
    } // end loop recipients

    if (!foundRecipient)
    {
      mdcmErrorMacro("No recipient found with the specified private key.");
      goto err;
    }
  }
  ...
  {
    mdcmErrorMacro("SetKeyParam KP_IV failed with error 0x" << std::hex << GetLastError());
    goto err;
  }

  {
    ...
    {
      mdcmErrorMacro("SetKeyParam KP_MODE failed with error 0x" << std::hex << GetLastError());
      goto err;
    }
  }
  ...
  {
    mdcmErrorMacro("MsgGetParam CMSG_BARE_CONTENT_PARAM failed with error 0x" << std::hex << GetLastError());
    goto err;
  }
  if (!CryptDecrypt(hCEK, 0, TRUE, 0, bareContent, &bareContentLen))
  {
    mdcmErrorMacro("CryptDecrypt failed with error 0x" << std::hex << GetLastError());
    goto err;
  }
  if (bareContentLen > outlen)
  {
    mdcmErrorMacro("Supplied output buffer too small: " << bareContentLen << " bytes needed.");
    goto err;
  }
...
ALG_ID
CAPICryptographicMessageSyntax::GetAlgIdByObjId(const char * pszObjId)
{
...
const char *
CAPICryptographicMessageSyntax::GetCipherObjId() const
{
  switch (cipherType)
  {
    case AES128_CIPHER:
...
bool
CAPICryptographicMessageSyntax::Initialize()
{
  DWORD dwResult;
// FIXME
#if 1
  ...
  // CRYPT_VERIFYCONTEXT aes decr in cryptmsgcontrol not working
  {
    ...
    if (dwResult == NTE_BAD_KEYSET)
    {
      ...
      {
        dwResult = GetLastError();
        mdcmErrorMacro("CryptAcquireContext() failed:" << std::hex << dwResult);
        return false;
      }
    }
    else if (dwResult == NTE_KEYSET_NOT_DEF)
    {
      ...
      if (!CryptAcquireContextA(&hProv,
                                ...
                                PROV_RSA_AES,
                                CRYPT_VERIFYCONTEXT)) // CRYPT_VERIFYCONTEXT aes decr in cryptmsgcontrol not working
      {
        dwResult = GetLastError();
        if (dwResult == NTE_BAD_KEYSET)
        {
...
void
CAPICryptographicMessageSyntax::ReverseBytes(BYTE * data, DWORD len)
{
  BYTE temp;
...
bool
CAPICryptographicMessageSyntax::LoadFile(const char * filename, BYTE *& buffer, DWORD & bufLen)
{
  assert(!buffer);
