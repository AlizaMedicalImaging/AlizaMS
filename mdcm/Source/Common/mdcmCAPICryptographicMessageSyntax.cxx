...
bool CAPICryptographicMessageSyntax::ParseCertificateFile(const char * filename)
{
  ...
  if (!LoadFile(filename, certHexBuf, certHexBufLen)) goto err;
...
bool CAPICryptographicMessageSyntax::ParseKeyFile( const char *filename ) {
  ...
  {
    mdcmErrorMacro("Failed to convert from BASE64. CryptStringToBinary failed with error 0x" << std::hex << GetLastError());
    goto err;
  }
  ...
  {
    mdcmErrorMacro( "Failed to parse private key. CryptDecodeObjectEx failed with error 0x" << std::hex << GetLastError() );
    goto err;
  }
...
bool CAPICryptographicMessageSyntax::Decrypt(char *output, size_t &outlen, const char *array, size_t len) const 
{
  ...
  DWORD cekLen;
  {
    ...
    for (DWORD i=0; i < nrOfRecipeints; i++)
    {
      ...
      {
        mdcmErrorMacro("MsgGetParam CMSG_CMS_RECIPIENT_INFO_PARAM size failed with error 0x" << std::hex << GetLastError());
        goto err;
      }
      ...
      if ((foundRecipient = CryptDecrypt(hRsaPrivK, 0, TRUE, rsaPadding, cek, &cekLen))) break;
    } // end loop recipients
...
// FIXME
  ...
  //CRYPT_VERIFYCONTEXT aes decr in cryptmsgcontrol not working
  {
    ...
    {
#if 1
      if (!CryptAcquireContextA(&hProv, NULL, MS_ENH_RSA_AES_PROV_A, PROV_RSA_AES, CRYPT_NEWKEYSET | CRYPT_VERIFYCONTEXT))
#else
