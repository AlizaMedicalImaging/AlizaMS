...
private:
  bool
                    zip_to_stream(char_type * buffer, std::streamsize buffer_size);
...
private:
  ...
  std::streamsize
  unzip_from_stream(char_type * buffer, std::streamsize buffer_size);
  size_t
                    fill_input_buffer();
...
public:
  ...
  inline explicit basic_zip_ostream(ostream_reference ostream,
                                    bool              is_gzip = false,
                                    int               level = Z_DEFAULT_COMPRESSION,
                                    EStrategy         strategy = DefaultStrategy,
                                    int    window_size = -15 /*windowBits is passed < 0 to suppress zlib header */,
                                    int    memory_level = 8,
                                    size_t buffer_size = zstream_default_buffer_size * 8);
  ...
  void
  finished();
  // void make_gzip() { add_header(); _is_gzip = true; }
