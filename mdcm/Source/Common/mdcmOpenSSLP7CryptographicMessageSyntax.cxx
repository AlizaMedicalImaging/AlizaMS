...
public:
  ...
  STACK_OF(X509) * GetRecipients() const { return recips; }

  ::X509 *
  GetRecipient(unsigned int i) const
  {
  ...
  void
  SetPrivateKey(::EVP_PKEY * thepkey)
  {
    this->pkey = thepkey;
  }
  ...
  void
  SetCipherType(CryptographicMessageSyntax::CipherTypes ciphertype)
  {
  ...
  Initialize()
  {
    ...
    for (unsigned int i = 0; i < GetNumberOfRecipients(); ++i)
    {
      ...
      if (!PKCS7_add_recipient(p7, recip))
        return false;
    }
  ...
  Encrypt(char * output, size_t & outlen, const char * array, size_t len)
  {
    ...
    if (!data)
    {
      mdcmErrorMacro("BIO_new_mem_buf");
      return false;
    }
    char  buf[256];
    BIO * p7bio;
    ...
    if (bflush != 1)
    {
      mdcmErrorMacro("BIO_flush: " << bflush);
      return false;
    }
    if (!PKCS7_dataFinal(p7, p7bio))
    {
      mdcmErrorMacro("PKCS7_dataFinal");
      return false;
    }
    // WARNING:
...
void
OpenSSLP7CryptographicMessageSyntax::SetCipherType(CryptographicMessageSyntax::CipherTypes type)
{
#ifdef MDCM_USE_SYSTEM_OPENSSL
...
CryptographicMessageSyntax::CipherTypes
OpenSSLP7CryptographicMessageSyntax::GetCipherType() const
{
#ifdef MDCM_USE_SYSTEM_OPENSSL
...
bool
OpenSSLP7CryptographicMessageSyntax::Encrypt(char * output, size_t & outlen, const char * array, size_t len) const
{
#ifdef MDCM_USE_SYSTEM_OPENSSL
...
*/
bool
OpenSSLP7CryptographicMessageSyntax::Decrypt(char * output, size_t & outlen, const char * array, size_t len) const
{
#ifdef MDCM_USE_SYSTEM_OPENSSL
...
bool
OpenSSLP7CryptographicMessageSyntax::ParseKeyFile(const char * keyfile)
{
#ifdef MDCM_USE_SYSTEM_OPENSSL
  ::BIO *      in;
  ::EVP_PKEY * pkey;
...
bool
OpenSSLP7CryptographicMessageSyntax::ParseCertificateFile(const char * keyfile)
{
#ifdef MDCM_USE_SYSTEM_OPENSSL
