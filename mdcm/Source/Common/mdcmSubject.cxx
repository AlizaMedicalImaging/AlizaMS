...
{
public:
  Observer(Command * c, const Event * event, unsigned long tag)
    : m_Command(c)
    , m_Event(event)
    , m_Tag(tag)
  {}
...
public:
  ...
  unsigned long
  AddObserver(const Event & event, Command * cmd) const;
  void
  RemoveObserver(unsigned long tag);
  void
  RemoveAllObservers();
  void
  InvokeEvent(const Event & event, Subject * self);
  void
  InvokeEvent(const Event & event, const Subject * self);
  Command *
...
unsigned long
SubjectInternals::AddObserver(const Event & event, Command * cmd)
{
...
unsigned long
SubjectInternals::AddObserver(const Event & event, Command * cmd) const
{
  Observer *         ptr = new Observer(cmd, event.MakeObject(), m_Count);
...
void
SubjectInternals::RemoveObserver(unsigned long tag)
{
  for (std::list<Observer *>::iterator i = m_Observers.begin(); i != m_Observers.end(); ++i)
  {
    if ((*i) && (*i)->m_Tag == tag)
...
void
SubjectInternals::RemoveAllObservers()
{
  for (std::list<Observer *>::iterator i = m_Observers.begin(); i != m_Observers.end(); ++i)
  {
    if (*i)
...
void
SubjectInternals::InvokeEvent(const Event & event, Subject * self)
{
  for (std::list<Observer *>::iterator i = m_Observers.begin(); i != m_Observers.end(); ++i)
  {
...
void
SubjectInternals::InvokeEvent(const Event & event, const Subject * self)
{
  for (std::list<Observer *>::iterator i = m_Observers.begin(); i != m_Observers.end(); ++i)
  {
    const Event * e = (*i)->m_Event;
...
Command *
SubjectInternals::GetCommand(unsigned long tag)
{
  for (std::list<Observer *>::iterator i = m_Observers.begin(); i != m_Observers.end(); ++i)
  {
    if ((*i) && (*i)->m_Tag == tag)
...
unsigned long
Subject::AddObserver(const Event & event, Command * cmd)
{
  return this->Internals->AddObserver(event, cmd);
}
unsigned long
Subject::AddObserver(const Event & event, Command * cmd) const
{
  return this->Internals->AddObserver(event, cmd);
}

Command *
Subject::GetCommand(unsigned long tag)
{
  return this->Internals->GetCommand(tag);
}

void
Subject::InvokeEvent(const Event & event)
{
  this->Internals->InvokeEvent(event, this);
}

void
Subject::InvokeEvent(const Event & event) const
{
  this->Internals->InvokeEvent(event, this);
}

void
Subject::RemoveObserver(unsigned long tag)
{
...
bool
Subject::HasObserver(const Event & event) const
{
