...
public:
  ...
  m_Event(event),
  m_Tag(tag) {}
  virtual ~Observer() { if (m_Event) delete m_Event; }
...
unsigned long SubjectInternals::AddObserver(const Event & event, Command * cmd)
{
...
unsigned long SubjectInternals::AddObserver(const Event & event, Command * cmd) const
{
  Observer * ptr = new Observer(cmd, event.MakeObject(), m_Count);
...
void SubjectInternals::RemoveObserver(unsigned long tag)
{
  for(std::list<Observer* >::iterator i = m_Observers.begin();
      i != m_Observers.end();
      ++i)
...
void SubjectInternals::RemoveAllObservers()
{
  for(std::list<Observer* >::iterator i = m_Observers.begin();
      i != m_Observers.end();
      ++i)
...
void SubjectInternals::InvokeEvent(const Event & event, Subject * self)
{
  for(std::list<Observer* >::iterator i = m_Observers.begin();
      i != m_Observers.end(); ++i)
  {
...
void SubjectInternals::InvokeEvent(const Event & event, const Subject * self)
{
  for(std::list<Observer* >::iterator i = m_Observers.begin();
      i != m_Observers.end(); ++i)
  {
...
Command * SubjectInternals::GetCommand(unsigned long tag)
{
  for(std::list<Observer* >::iterator i = m_Observers.begin();
      i != m_Observers.end(); ++i)
  {
...
bool SubjectInternals::HasObserver(const Event & event) const
{
...
bool SubjectInternals::PrintObservers(std::ostream & os, std::string indent) const
{
