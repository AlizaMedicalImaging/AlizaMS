...
bool PixmapReader::Read()
{
  ...
  else
  {
    ...
    {
      mdcmDebugMacro("DICOM file is not an Image file but a : " <<
  ...
  for(size_t ov_idx = pixeldata.GetNumberOfOverlays(); ov_idx != 0; --ov_idx)
  {
    ...
    if(o.IsInPixelData())
    {
      ...
      if(obp < pf.GetBitsStored())
      {
        ...
        mdcmWarningMacro("Invalid BitPosition: " << obp << " for overlay #" <<
          ov << " removing it.");
      }
    }
  }
...
bool PixmapReader::ReadImageInternal(MediaStorage const &ms, bool handlepixeldata)
{
  ...
  {
    mdcmDebugMacro("NumberOfFrames was specified with a value of: "
      << numberofframes);
  ...
  if(photometricinterpretation)
  {
    ...
    {
      mdcmWarningMacro("Discarding suspicious PhotometricInterpretation found: "
  ...
  // IconImage
  if (m_ProcessIcons) DoIconImage(ds, *PixelData);
  // Curves
  if (m_ProcessCurves) DoCurves(ds, *PixelData);
  // Overlays
  if (m_ProcessOverlays) DoOverlays(ds, *PixelData);
  // PixelData
  if(handlepixeldata)
  {
     if(pixeldata_)
