...
std::vector<float>
SurfaceHelper::RGBToXYZ(const std::vector<float> & RGB)
{
  ...
  if (tmp[0] > 0.04045f)
    tmp[0] = powf((tmp[0] + 0.055f) * B, 2.4f);
  else
    tmp[0] *= A;
  if (tmp[1] > 0.04045f)
    tmp[1] = powf((tmp[1] + 0.055f) * B, 2.4f);
  else
    tmp[1] *= A;
  if (tmp[2] > 0.04045f)
    tmp[2] = powf((tmp[2] + 0.055f) * B, 2.4f);
  else
    tmp[2] *= A;
  tmp[0] *= 100;
  tmp[1] *= 100;
...
std::vector<float>
SurfaceHelper::XYZToRGB(const std::vector<float> & XYZ)
{
  ...
  if (RGB[0] > 0.0031308f)
    RGB[0] = 1.055f * powf(RGB[0], A) - 0.055f;
  else
    RGB[0] *= 12.92f;
  if (RGB[1] > 0.0031308f)
    RGB[1] = 1.055f * powf(RGB[1], A) - 0.055f;
  else
    RGB[1] *= 12.92f;
  if (RGB[2] > 0.0031308f)
    RGB[2] = 1.055f * powf(RGB[2], A) - 0.055f;
  else
    RGB[2] *= 12.92f;
  return RGB;
}
...
std::vector<float>
SurfaceHelper::XYZToCIELab(const std::vector<float> & XYZ)
{
  ...
  if (tmp[0] > 0.008856f)
    ...
    tmp[0] = (7.787f * tmp[0] + B);
  if (tmp[1] > 0.008856f)
    tmp[1] = powf(tmp[1], A);
  else
    tmp[1] = (7.787f * tmp[1] + B);
  if (tmp[2] > 0.008856f)
    tmp[2] = powf(tmp[2], A);
  else
    tmp[2] = (7.787f * tmp[2] + B);
...
std::vector<float>
SurfaceHelper::CIELabToXYZ(const std::vector<float> & CIELab)
{
  std::vector<float> XYZ(3);
  float              tmp[3];
  ...
  if (B > 0.008856f)
    ...
    tmp[1] = (tmp[1] - D) / 7.787f;
  if (A > 0.008856f)
    tmp[0] = A;
  else
    tmp[0] = (tmp[0] - D) / 7.787f;
  if (C > 0.008856f)
    tmp[2] = C;
  else
    tmp[2] = (tmp[2] - D) / 7.787f;
