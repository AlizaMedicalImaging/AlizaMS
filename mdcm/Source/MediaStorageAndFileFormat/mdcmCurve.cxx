  ...
  while(!finished)
    ...
    else
    {
      if(ds.FindDataElement(Tag(de.GetTag().GetGroup(),0x3000)))
      {
        ...
        if(!overlaydata.IsEmpty()) { ++numoverlays; }
      }
...
const char * Curve::GetTypeOfDataDescription() const
{
  ...
  while(p)
  {
    if(Internal->TypeOfData == p) { break; }
    ++i;
    p = t[i][0];
  }
  return t[i][1];
...
{
  if(curvedescription) Internal->CurveDescription = curvedescription;
}

void Curve::SetDataValueRepresentation(unsigned short datavaluerepresentation)
{
  Internal->DataValueRepresentation = datavaluerepresentation;
}

unsigned short Curve::GetDataValueRepresentation() const
{
...
void Curve::Decode(std::istream &, std::ostream &)
{
  assert(0);
}
