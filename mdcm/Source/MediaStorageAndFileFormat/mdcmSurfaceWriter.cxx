...
SurfaceWriter::SurfaceWriter() : NumberOfSurfaces(0)
{
}
...
bool SurfaceWriter::PrepareWrite()
{
  ...
  if(nbSurfaces == 0)
  {
    mdcmErrorMacro("No surface to write");
    return false;
  }
  ...
  for (; it0 != it0End; ++it0)
  {
    ...
    for (; it1 != it1End; ++it1)
    {
      ...
      if(surfaceProcessing)
      {
        ...
        /* Surface Processing Algorithm Identification Sequence */
        {
          ...
          // See: PS.3.3 Table 8.8-1 and PS 3.16 Context ID 7162
          {
            ...
            {
              mdcmWarningMacro("Surface processing algorithm family not specified or incomplete");
            }
        ...
        && primitiveType != MeshPrimitive::TRIANGLE_FAN)
      {
        ...
        VL vl;
        if(bv)vl = bv->GetLength();
        else vl.SetToUndefined();
        vectorCoordDataDE.SetVL(vl);
        if(ts.IsExplicit()) vectorCoordDataDE.SetVR(VR::OF);
        surfacePointsNormalsDS.Replace(vectorCoordDataDE);
      }
      else if(numberofvectors > 0)
      {
        mdcmWarningMacro("Triangle strip or fan have no surface points normals");
      }
      ...
      // Surface Mesh Primitives Sequence
      {
        ...
        if(insertInSQ)
        {
          ...
          for (; it != itEnd; ++it)
          {
            ...
            if(ts.IsExplicit()) typedPointIndexListDE.SetVR(VR::OL);
        ...
        else
        {
          ...
          const ByteValue * pointIndexListBV = typedPointIndexListDE.GetByteValue();
          VL pointIndexListVL;
          if(pointIndexListBV) pointIndexListVL = pointIndexListBV->GetLength();
          else pointIndexListVL.SetToUndefined();
          typedPointIndexListDE.SetVL(pointIndexListVL);
          if(ts.IsExplicit()) typedPointIndexListDE.SetVR(VR::OL);
    ...
    for (; itRefSurface != itEndRefSurface; ++itRefSurface)
    {
      ...
      /* Segment Surface Generation Algorithm Identification Sequence */
      {
        ...
        {
          mdcmWarningMacro("Segment surface generation algorithm family not specified or incomplete");
        }
        SmartPointer<SequenceOfItems> algoFamilyCodeSQ;
        const Tag algoFamilyCodeTag(0x0066, 0x002F);
        ...
        {
          mdcmWarningMacro("No algorithm version specified");
        }
        ...
        {
          mdcmWarningMacro("No algorithm name specified");
        }
...
bool SurfaceWriter::PrepareWritePointMacro(
 SmartPointer<Surface> surface, DataSet & surfaceDS, const TransferSyntax & ts)
{
  /* Surface Points  */
  // (0066,0011) SQ (Sequence with undefined length #=1) # u/l, 1 Surface Points Sequence
  //   (fffe,e000) na (Item with undefined length #=1)   # u/l, 1 Item
  ...
  /* Surface Points Sequence */
  {
    ...
    if(ts.IsExplicit()) pointCoordDataDE.SetVR(VR::OF);
