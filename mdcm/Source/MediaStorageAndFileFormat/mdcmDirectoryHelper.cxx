...
Directory::FilenamesType DirectoryHelper::GetSeriesUIDsBySOPClassUID(
  ...
  const std::string& inSOPClassUID)
{
  ...
  try
  {
    ...
    for (size_t i = 0; i < theNumSeries; i++)
    {
      ...
      size_t endpos = theSOPClassUID.find_last_not_of(" ");
...
Directory::FilenamesType DirectoryHelper::GetFilenamesFromSeriesUIDs(
  const std::string& inDirectory,
  const std::string& inSeriesUID)
{
  ...
  try
  {
    ...
    for (size_t i = 0; i < theNumSeries; i++)
    {
      ...
      // DICOM strings sometimes have trailing spaces; make sure to avoid those.
      // Find the first character position from reverse af.
      size_t endpos = theSeriesUID.find_last_not_of(" "); 
      if(std::string::npos != endpos)
      {
                                                           ...
                                                           const std::string& inSeriesUID)
{
  ...
  try
  {
    ...
    for (size_t i = 0; i < theNumSeries; i++)
    {
      if (inSeriesUID == theSeriesValues[i])
      {
        ...
        {
          mdcmWarningMacro("Unable to sort Image Files.");
          return blank;
        }
  ...
  {
    mdcmWarningMacro("Something went wrong reading CT images.");
    return blank;
  }
}
...
// Each plane needs to know the SOPClassUID, and that won't change
// from image to image so, retrieve this once at the start of writing.
