...
public:
  ...
  bool StopDataElement(const Tag & t)
  {
    ...
    if(CheckPixelDataElement)
    {
      ...
      {
        mdcmDebugMacro("Only RAW (uncompressed) Pixel Data is supported for now");
        return false;
      }
      ...
      if(ba.GetValue() % 8 != 0)
      {
        mdcmErrorMacro("old ACR NEMA file: " << ba.GetValue());
        return false;
      }
      ...
      {
        mdcmDebugMacro("Invalid size for Pixel Data Element: " << computedlength << " vs " << currentdatalenth);
        return false;
      }
    }
    ...
    while(len_to_move != 0)
    {
      ...
      if(CurrentDataLenth == MaxSizeDE)
      {
        ...
        if(lowbits == 0)
        {
          ...
          mdcmDebugMacro("Too many data elements. Giving up");
          return false;
        }
      }
    }
...
private:
  bool UpdateDataElement(const Tag & t)
  {
    ...
    if(CurrentDataLenth)
    {
      ...
      vlpos -= 4; // Tag
      mdcmAssertAlwaysMacro(vlpos >= 0);
      const Tag tag = t;
      mdcmAssertAlwaysMacro(CurrentDataLenth < std::numeric_limits<uint32_t>::max());
  ...
  size_t WriteHelper(off64_t offset, const Tag & tag, const VL & vl)
  {
    ...
    std::stringstream ss;
    if(TS.GetSwapCode() == SwapCode::BigEndian) tag.Write<SwapperDoOp>(ss);
    else tag.Write<SwapperNoOp>(ss);
    if(TS.GetNegociatedType() == TransferSyntax::Explicit)
    {
      VR un = VR::UN;
      un.Write(ss);
    }
    if(TS.GetSwapCode() == SwapCode::BigEndian) vl.Write<SwapperDoOp>(ss);
    else vl.Write<SwapperNoOp>(ss);
...
bool FileStreamer::StartDataElement(const Tag & t)
{
  ...
  {
    mdcmDebugMacro("Could not StartDataElement");
    return false;
  }
  if(!InitializeCopy())
  {
    mdcmDebugMacro("Could not InitializeCopy");
    return false;
  }
...
bool FileStreamer::AppendToDataElement(const Tag & t, const char *data, size_t len)
{
  ...
  {
    mdcmDebugMacro("Could not AppendToDataElement");
    return false;
  }
...
bool FileStreamer::StartGroupDataElement(const PrivateTag & pt, size_t maxsizede, uint8_t startoffset)
{
  ...
  {
    mdcmDebugMacro("Could not StartGroupDataElement");
    return false;
  }
  if(!InitializeCopy())
  {
...
bool FileStreamer::AppendToGroupDataElement(const PrivateTag & pt, const char *data, size_t len)
{
  ...
  {
    mdcmDebugMacro("Could not AppendToGroupDataElement");
    return false;
  }
