 ...
 *
 *=========================================================================*/
#include "mdcmStreamImageReader.h"
...
bool StreamImageReader::ReadImageSubregionRAW(char* inReadBuffer, const std::size_t& inBufferLength)
{
  ...
  if( !theCodec.CanDecode(ts) )
    ...
    if (!theJpegLSCodec.CanDecode(ts))
      {
      mdcmDebugMacro( "Raw Codec cannot decode this file by streaming." );
      return false;
      } 
    else 
      {
  ...
  try
    ...
    {
    (void)ex;
    mdcmWarningMacro( "Failed to read with ex:" << ex.what() );
    delete [] tmpBuffer;
    delete [] tmpBuffer2;
    return false;
    } 
    ...
    {
    mdcmWarningMacro( "Failed to read with unknown error." );
    delete [] tmpBuffer;
    delete [] tmpBuffer2;
    return false;
    }
...
bool StreamImageReader::ReadImageSubregionJpegLS(char* inReadBuffer, const std::size_t& inBufferLength) {
  ...
  if (!theCodec.CanDecode(ts))
  {
    mdcmDebugMacro( "JpegLS cannot read this." );
    return false;
  }
  //read in the entire file for jpegls
  //right now, it needs to be read entirely off of disk first
...
bool StreamImageReader::ReadImageInformation()
{
  ...
  try
    ...
    if (!mReader.ReadUpToTag(thePixelDataTag, theSkipTags))
      {
      mdcmWarningMacro("Failed to read tags in the mdcm stream image reader.");
      return false;
      }
#if 0
  ...
  if( ms == MediaStorage::VLWholeSlideMicroscopyImageStorage )
    ...
    if( !ds_t.FindDataElement( Tag(0x0048,0x0200) ) )
      {
      mdcmWarningMacro( "error occurred in WSI File read" );
      return false;
      }
    ...
    if( !subds_L.FindDataElement( Tag(0x0008,0x1160) ) )
      {
      mdcmWarningMacro( "Error occurred during WSI File Read" );
      return false;
      }
    ...
    if( !subds_L.FindDataElement( Tag(0x0048,0x0202) ) )
      {
      mdcmWarningMacro( "Error During WSI File Read" );
      return false;
      }
