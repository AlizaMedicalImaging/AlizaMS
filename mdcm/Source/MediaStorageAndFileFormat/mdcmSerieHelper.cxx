...
void SerieHelper::Clear()
{
  ...
  while (l)
  {
    ...
    for (FileList::iterator it  = l->begin();
      ...
      ++it)
    {
      //delete *it; // remove each entry FIXME
    }
    l->clear();
...
void SerieHelper::AddFileName(std::string const &filename)
{
  ...
  if(!reader.Read())
  {
    mdcmWarningMacro("Could not read file: " << filename);
  }
  else
  {
...
static bool CompareDicomString(const std::string &s1, const char *s2, int op)
{
  ...
  case MDCM_GREATER :
    return s1_even >  s2_even;
  case MDCM_GREATEROREQUAL :
    return s1_even >= s2_even;
  case MDCM_LESS :
    return s1_even <  s2_even;
  case MDCM_LESSOREQUAL :
    return s1_even <= s2_even;
  default :
    mdcmDebugMacro(" Wrong operator : " << op);
    return false;
  }
}
bool SerieHelper::AddFile(FileWithName &header)
{
    ...
    ++it2)
  {
    ...
    {
      // This rule is unmatched; let's just quit
      allrules = 0;
      break;
    }
  }
  ...
  if (allrules) // all rules are respected
  {
    ...
    // if id == MDCM_UNFOUND then consistently we should find MDCM_UNFOUND
    // no need here to do anything special

    if (SingleSerieUIDFileSetHT.count(id) == 0)
    {
      mdcmDebugMacro(" New Serie UID :[" << id << "]");
...
bool SerieHelper::ImagePositionPatientOrdering(FileList *fileList)
{
  ...
  if (min == max)
  {
    mdcmWarningMacro("Looks like all images have the exact same image position"
...
std::string SerieHelper::CreateUniqueSeriesIdentifier(File * inFile)
{
  ...
  {
    mdcmWarningMacro("Could not parse series info.");
    std::string id = "MDCM_UNFOUND";
    return id;
  }
}
