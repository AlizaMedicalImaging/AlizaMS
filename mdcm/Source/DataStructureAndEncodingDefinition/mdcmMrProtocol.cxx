...
bool
MrProtocol::Load(const ByteValue * bv, const char * csastr, int version)
{
  if (bv)
  {
    ...
    // ### ASCCONV BEGIN ###
    // as well as:
    // ### ASCCONV BEGIN object=MrProtDataImpl@MrProtocolData version=41310008
...
const char *
MrProtocol::GetMrProtocolByName(const char * name) const
{
  if (name)
  {
    const MyMapType &         mymap = Pimpl->mymap;
...
bool
MrProtocol::FindMrProtocolByName(const char * name) const
{
  if (name)
  {
    const MyMapType &         mymap = Pimpl->mymap;
...
bool
MrProtocol::GetSliceArray(MrProtocol::SliceArray & sa) const
{
