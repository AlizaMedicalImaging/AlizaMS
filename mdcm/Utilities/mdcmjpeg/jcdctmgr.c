...
#  define CONST_BITS 14
        ...
        for (i = 0; i < DCTSIZE2; i++)
        {
          dtbl[i] = (DCTELEM)DESCALE(MULTIPLY16V16((IJG_INT)qtbl->quantval[i], (IJG_INT)aanscales[i]), CONST_BITS - 3);
        }
      }
      break;
#endif
      ...
      case JDCT_FLOAT:
      {
        ...
        if (fdct->float_divisors[qtblno] == NULL)
        {
          fdct->float_divisors[qtblno] =
            (FAST_FLOAT *)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, DCTSIZE2 * SIZEOF(FAST_FLOAT));
        }
        ...
        for (row = 0; row < DCTSIZE; row++)
        {
          ...
          {
            fdtbl[i] =
              (FAST_FLOAT)(1.0 / (((double)qtbl->quantval[i] * aanscalefactor[row] * aanscalefactor[col] * 8.0)));
            i++;
          }
        }
      }
...
jinit_forward_dct(j_compress_ptr cinfo)
{
  ...
  fdct = (fdct_ptr)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, SIZEOF(fdct_controller));
