                         ...
                         JDIMENSION       out_rows_avail)
{
  ...
  (*cinfo->post->post_process_data)(
    cinfo, mainPtr->buffer, &mainPtr->rowgroup_ctr, rowgroups_avail, output_buf, out_row_ctr, out_rows_avail);
                          ...
                          JDIMENSION       out_rows_avail)
{
  ...
  if (!mainPtr->buffer_full)
  {
    if (!(*cinfo->codec->decompress_data)(cinfo, mainPtr->xbuffer[mainPtr->whichptr]))
      return;                    /* suspension forced, can do nothing more */
    mainPtr->buffer_full = TRUE; /* OK, we have an iMCU row to work with */
  ...
  switch (mainPtr->context_state)
  {
    case CTX_POSTPONED_ROW:
      ...
      (*cinfo->post->post_process_data)(cinfo,
                                        ...
                                        &mainPtr->rowgroup_ctr,
                                        mainPtr->rowgroups_avail,
                                        output_buf,
                                        out_row_ctr,
                                        out_rows_avail);
