...
METHODDEF(void)
simple_upscale(j_decompress_ptr cinfo, JDIFFROW diff_buf, JSAMPROW output_buf, JDIMENSION width)
{
  j_lossless_d_ptr losslsd = (j_lossless_d_ptr)cinfo->codec;
  scaler_ptr       s = (scaler_ptr)losslsd->scaler_private;
...
METHODDEF(void)
simple_downscale(j_decompress_ptr cinfo, JDIFFROW diff_buf, JSAMPROW output_buf, JDIMENSION width)
{
  j_lossless_d_ptr losslsd = (j_lossless_d_ptr)cinfo->codec;
  scaler_ptr       s = (scaler_ptr)losslsd->scaler_private;
  int              scale_factor = s->scale_factor;
...
METHODDEF(void)
noscale(j_decompress_ptr cinfo, JDIFFROW diff_buf, JSAMPROW output_buf, JDIMENSION width)
{
  unsigned int xindex;
  (void)cinfo;
...
scaler_start_pass(j_decompress_ptr cinfo)
{
   ...
   */
  downscale = BITS_IN_JSAMPLE < cinfo->data_precision ? cinfo->data_precision - BITS_IN_JSAMPLE : 0;

  s->scale_factor = cinfo->Al - downscale;

  /* Set scaler functions based on scale_factor (positive = left shift) */
  if (s->scale_factor > 0)
    losslsd->scaler_scale = simple_upscale;
  else if (s->scale_factor < 0)
  {
...
jinit_d_scaler(j_decompress_ptr cinfo)
{
  ...
  s = (scaler_ptr)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, SIZEOF(scaler));
  losslsd->scaler_private = (void *)s;
